...
static void ImGui_ImplWiiU_AppearKeyboardInput()
{
    ImGui_ImplWiiU_Data* bd = ImGui_ImplWiiU_GetBackendData();
...
static void ImGui_ImplWiiU_UpdateKeyboardInput(const ImGui_ImplWiiU_ControllerInput* input)
{
    ImGuiIO& io = ImGui::GetIO();
    ImGuiInputTextState* state = ImGui::GetInputTextState(ImGui::GetActiveID());
    if (!state)
    ...
    {
        vpad = *input->vpad;
        VPADGetTPCalibratedPoint(VPAD_CHAN_0, &vpad.tpNormal, &input->vpad->tpNormal);
    }

    nn::swkbd::ControllerInfo controllerInfo;
    controllerInfo.vpad = input->vpad ? &vpad : NULL;
    for (int i = 0; i < 4; i++)
        controllerInfo.kpad[i] = (KPADStatus*)input->kpad[i];
...
static void ImGui_ImplWiiU_UpdateTouchInput(const ImGui_ImplWiiU_ControllerInput* input)
{
    if (!input->vpad)
        return;
...
static void ImGui_ImplWiiU_UpdateControllerInput(const ImGui_ImplWiiU_ControllerInput* input)
{
    ImGui_ImplWiiU_Data* bd = ImGui_ImplWiiU_GetBackendData();
    ImGuiIO& io = ImGui::GetIO();
    ...
    for (int i = 0; i < 4; i++)
    {
        const KPADStatus* kpad = input->kpad[i];
        if (!kpad)
            continue;
...
bool     ImGui_ImplWiiU_ProcessInput(const ImGui_ImplWiiU_ControllerInput* input)
{
    ImGui_ImplWiiU_Data* bd = ImGui_ImplWiiU_GetBackendData();
    ...
    if (io.WantTextInput && !bd->WantedTextInput) 
        ImGui_ImplWiiU_AppearKeyboardInput();
